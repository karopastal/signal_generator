...
def generate_record(samples, probabilities, rebin_shape, signal_id=0, bg_id=0, wavelet_id=0):
    ...
    rebined_cwt_bg_record = rebin(cwt_bg_record, rebin_shape)
    record = calculate_p_value_matrix(rebined_cwt_bg_record, samples, probabilities)
...
def create_train_dataset(samples, probabilities):
    ...
    for i in range(TRAIN_SIZE):
        ...
        # rebined_cwt_bg_record = rebin(cwt_bg_record, REBINED_SHAPE)
        # record = calculate_p_value_matrix(rebined_cwt_bg_record, samples, probabilities)
...
def create_test_signals_datasets(samples, probabilities):
    ...
    for signal_id in range(1, SIGNALS_NUM + 1):
        ...
        for i in range(TEST_SIGNALS_SIZE):
            ...
            #                                               wavelet_id=0)
            #
            # rebined_cwt_signal_record = rebin(cwt_signal_record, REBINED_SHAPE)
            # record = calculate_p_value_matrix(rebined_cwt_signal_record, samples, probabilities)
